...
class SaleItemsModel extends DbConfig
{
    ...
    public function getRoundingTotal($stime, $etime){
        $sql = "SELECT COALESCE(SUM(rounding), 0) as total, COALESCE(COUNT(id), 0) as num, COALESCE(GROUP_CONCAT(ref ),'') as refs FROM sales WHERE (processdt>= :stime AND processdt<= :etime) AND rounding!=0";
        $placeholders = [":stime"=>$stime, ":etime"=>$etime];
